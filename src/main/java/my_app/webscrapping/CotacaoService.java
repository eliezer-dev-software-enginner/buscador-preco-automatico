...
public class CotacaoService {
    ...
    private static final int PNCP_MAX_PAGINAS       = 2;   // ↑ para mais resultados, ↓ para mais velocidade
    private static final int PNCP_JANELA_DIAS       = 180;
    ...
    private List<ResultadoCotacao> buscarComScore(
        ...
        try {
            String fonte = nomeFonte(scraper.urlBase);
    ...
    private ResultadoCotacao buscarMelhorPncp(String tituloBusca, String palavrasChave) {
        try {
            // Usa a versão configurável do PNCP com limite de páginas
            PncpPesquisaPreco pncp = new PncpPesquisaPreco();
            DateTimeFormatter fmt = DateTimeFormatter.ofPattern("yyyyMMdd");
            String hoje    = LocalDate.now().format(fmt);
            String passado = LocalDate.now().minusDays(PNCP_JANELA_DIAS).format(fmt);

            List<PncpPesquisaPreco.ItemEncontrado> itens = pncp.pesquisarPrecoItem(
                    tituloBusca, passado, hoje, PNCP_MAX_PAGINAS,
                    PncpPesquisaPreco.Modalidade.PREGAO_ELETRONICO,
                    PncpPesquisaPreco.Modalidade.DISPENSA
            );

            if (itens.isEmpty()) return ResultadoCotacao.vazio("PNCP");
            ...
            return itens.stream()
                    ...
                    .orElse(ResultadoCotacao.vazio("PNCP"));

        } catch (Exception e) {
            return ResultadoCotacao.vazio("PNCP");
        }
    }

    /** Extrai nome legível da URL base: "https://www.loganmed.com.br" → "loganmed.com.br" */
    private String nomeFonte(String urlBase) {
